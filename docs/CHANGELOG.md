...
- **Fixed — weight field unreachable when creating a circuit.** In `CircuitBuilderSheet` the Rounds/Reps/Weight steppers sat in a non-wrapping flex row that overflowed narrow phones, pushing the Weight field off the right edge with no horizontal scroll — it looked like there was no weight field at all. The row now wraps (Weight drops to a visible second line), and the weight input accepts decimals (kg), which the create schema already allowed.
