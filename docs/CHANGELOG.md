...
- **Deferred:** **frontend** to create a circuit in the template builder + carry session circuits into a saved template (until that ships, nothing user-facing writes `template_exercises.circuit_id` — the remap paths are test-verified only); `targetWeightStep` on templates (needs a migration — template circuits carry no ramp yet); mixed-type circuits; per-exercise round counts; circuit-membership editing; **auto-ungroup on member delete** (a circuit dropping below 2 members should clear the survivors' `circuitId` server-side — read views already tolerate it; the backend `DELETE /session-exercises/:id` does not yet do it, and that same handler still lacks a trainer-ownership guard).
