...
- **What the first CI run of the matrix found (the lane itself came up clean — service container, `drizzle-kit push`, seeds):**
  - **`GET /templates/:id` returned 500 for any template whose exercise has media** — `serializeDates` converted only the top-level row; nested `exercise.media[].createdAt` stayed a `Date` and failed the response schema. Never hit in production only because no exercise has media yet; never hit in tests because factories already hold strings. Now a deep conversion.
