...
- **Present on all three routes to a session**, because a destructive action that exists on some routes but not others is worse than one that exists on none — you learn it is there, then cannot find it: the Sessions list card, the `SessionHistoryPanel` (opened from the dashboard's Recent Sessions and the sessions list), and the standalone `SessionHistoryPage` (still linked from the client-profile timeline and from the per-exercise history rows). Deleting from the panel or page closes/navigates back, since the view is showing a session that no longer exists.
- Shared `DeleteSessionButton` + `useDeleteSession` rather than three copies. The hook also drops the session from the client-side store and hides the overlay if it was focused, so a deleted session cannot leave a phantom pill or a stale "Resume" card on the dashboard — cleanup that is easy to get subtly different if duplicated.
- The Sessions card's action container already stopped click propagation, so deleting there cannot accidentally open the session history underneath.
