...
- **Completed sessions can now be deleted from the Sessions list.** A trash affordance on each completed card flips to a two-step `Keep` / `Delete` confirm — the same inline pattern the in-progress card already uses for Cancel, rather than a new modal. The backend route (`DELETE /sessions/:id`, trainer-scoped, cascades to workouts/exercises/sets) and the `useDiscardSession` hook already existed; this wires them to the one place a finished session is visible. The card's action container already stops click propagation, so deleting cannot accidentally open the session history underneath.
- On success the session is also dropped from the client-side store (and the overlay hidden if it was focused), so a deleted session cannot leave a phantom pill or a stale "Resume" card on the dashboard.
