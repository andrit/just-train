...
| 18 | Monitoring Setup | 🔄 **Current** — entered 2026-09-14. Code complete same day (L2 + L3 + L4-A): backend Sentry + Fastify handler + worker failure capture + cron heartbeat on the scheduler; SW errors relayed to Sentry; Vercel Speed Insights (20 %); first-party `client_events` counters via `POST /telemetry` behind a pluggable `TelemetrySink`. Human steps to close: two Sentry DSNs, uptime monitor, Railway alert, install-event check — `docs/user-tasks/phase-18-monitoring-setup.md` |
...
| 5 — Sheet/Modal (over nav) | Bottom sheets + bottom-anchored modals that must show while the nav is visible: `BottomSheet` (54 backdrop / 55 sheet), `SessionCloseout`, `TemplateExercisePickerSheet` | 54–55 |
