...
### Decision
**Fastify 5 applied 2026-09-16** (tier 2, first item) — deployed, matrix green, Sentry receiving from the new build. Sentry 10 applied 2026-09-17 → **57** (`@opentelemetry/core` gone; remaining runtime-reachable: `drizzle-orm` only). Re-audit after Fastify 5: **58** (3 low / 20 moderate / 33 high / 2 critical); `fastify`, `find-my-way`, `fast-uri`, `@fastify/static` gone. Remaining runtime-reachable: `drizzle-orm` (identifiers — low reachability), `@opentelemetry/core` via `@sentry/node` 8 (`baggage` header). Both criticals still install-time / dev-only.
